   ...
   for(my $j=1; $j<@peaksA;$j++)
   {
     ...
     for (my $i = 1; $i < @peaksB; $i++)
     {
       ...
       if ( abs(@$info_B[2] - $info_A[2]) < ((abs($info_A[3])+abs(@$info_B[3]))*$self->{peaks_coeff}) )
       {
          ...
          $matching_data{'centroid_dev'}        = Util::round((($val_B[0] - $info_A[0]) / $info_A[0])*100);
          $matching_data{'centroid_err_dev'}    = Util::round((($val_B[1] - $info_A[1]) / $info_A[1])*100);
          $matching_data{'energy_dev'}          = Util::round((($val_B[2] - $info_A[2]) / $info_A[2])*100);
          $matching_data{'energy_err_dev'}      = Util::round((($val_B[3] - $info_A[3]) / $info_A[3])*100);
          $matching_data{'area_dev'}            = Util::round((($val_B[8] - $info_A[8]) / $info_A[8])*100);
          $matching_data{'area_err_dev'}        = Util::round((($val_B[9] - $info_A[9]) / $info_A[9])*100);
