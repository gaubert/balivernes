...
sub is_true
{
   ...
   return 0 unless ( (exists  $self->{$group}) && (exists $self->{$group}->{$param}) );

   my $val = $self->{$group}->{$param};

   return ( ($val eq '1') || (lc($val) eq 'true') || (lc($val) eq 'yes') );
}
...
sub is_false
{
  ...
  return (! $self->is_true($group,$param));
}
...
sub exists_in_group
{
  ...
  {
    return (exists $self->{$group}->{$param})
  }
  else
  { 
    return 0;
...
sub get_group
{
   ...
   if (! exists  $self->{$group})
   {
      print STDOUT "Warning: $group doesn't exist in Conf \n" if $CONF_DEBUG;
...
sub get
{
   ...
   die "Error: $group doesn't exist" unless exists  $self->{$group};

   die "Error: $group".'['."$param"."] doesn't exist in conf file $self->{__file__}" unless exists $self->{$group}->{$param};

   # get value and if it is a list (contains ,) then create a list else return the val as a scalar
   #print "vals = $self->{$group}->{$param}\n";

   # split val
   my @val = split(',',$self->{$group}->{$param});
...
sub groups {
	...
	foreach $s ("__file__", "__default__", "__eol__", "__append__") {
		$i = 1;
		while ($i <= $#theGroups) {
			if ($s eq $theGroups[$i-1]) {
	      			splice(@theGroups, $i-1, 1);
			}
			$i++;
		}
	}
